...
def Train(mu):
	...
	for i in range(2,end+n):
		f[i-1]=5*np.sin(mu*(i-1))*yp[i-1]*yp[i-2]/(1+yp[i-1]**2 + yp[i-2]**2 + yp[i-3]**2)
		yp[i]=f[i-1]+u[i-1]+0.8*u[i-2]
	...
	# plt.figure()
	# plt.plot(endval[40000:40100],yp[40000:40100],color='red')
	# plt.plot(endval[40000:40100],yphat[40000:40100],color='blue')
	# plt.legend(["Plant","Model"])
	# plt.xlabel("Time Steps")
	# plt.ylabel("Amplitude")
	# plt.savefig("np90_8a_train.png")
	# plt.show()
	
	weights = [W1.tolist(), W2.tolist(), W3.tolist()]
	#weight_json = json.dumps(weights)
	with open("weights8a.json", 'w') as outfile:
