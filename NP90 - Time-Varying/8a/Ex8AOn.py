...
def Train():
    ...
    yp_OSLA=np.empty((end+n,1),dtype='float')
    ...
    for i in range(0,end+n):
        r[i] = np.sin(2*np.pi*(i)/25) #8a
        # r[i] = np.sin(2*np.pi*(i)/25)+np.sin(2*np.pi*(i)/10) #8b
    ...
    for a in range(epochs):
        for i in range(n,end+n):
            ...
            if((i-2)%Tc==0):
                ...
                uc_OSLA[i]=-fin -0.8*uc_OSLA[i-1] + 0.32*yp[i-1]+0.64*yp[i-2]-0.5*yp[i-3]+r[i-1]

            else:
                uc[i]=uc[i-1]
            
            yp[i]=np.sin(mu*(i-1))*f(yp[i-1],yp[i-2],yp[i-3])+uc[i]+0.8*uc[i-1]
            yp_OSLA[i]=np.sin(mu*(i-1))*f(yp_OSLA[i-1],yp_OSLA[i-2],yp_OSLA[i-3])+uc_OSLA[i]+0.8*uc_OSLA[i-1]
    ...
    plt.title(f"Example 8 Online Time-Varying mu = {mu} Initial Graph")
    plt.legend(["BPA","OSLA","Model"])
    plt.xlabel("Time Steps")
    plt.ylabel("Amplitude")
    plt.savefig("np90_8A_on_ini_TV2.png")
    ...
    plt.plot(endval[start:end],yp_OSLA[start:end],color='black')
    plt.plot(endval[start:end],ym[start:end],color='green',linewidth=0.8)
    plt.title(f"Example 8 Online Time-Varying mu = {mu} Final Graph")
    plt.legend(["BPA","OSLA","Model"])
    plt.xlabel("Time Steps")
    plt.ylabel("Amplitude")
    plt.savefig("np90_8A_on_fin_TV1.png")
    plt.show()
    return(W1,W2,W3)
