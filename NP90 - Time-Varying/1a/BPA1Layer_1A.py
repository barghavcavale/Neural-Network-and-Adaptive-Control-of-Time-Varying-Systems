...
def Run(mu):
	...
	for a in range(epochs):
		for i in range(2,end+n):
			if(i<(k+n)):
				...
				yp[i]=0.3*yp[i-1]+0.6*yp[i-2]+f[i]
				S=u[i-1]
				x=np.array([bias,S])
				A1=np.matmul(W1,x)
				A=tan_h(A1)
	...
	pickle_out=open("BPA1Layer_yphat.pickle","wb")
	pickle.dump(yphat,pickle_out)
	pickle_out.close()
	pickle_out=open("BPA1Layer_J.pickle","wb")
	...
	# plt.xlabel("Time")
	# plt.ylabel("Val")
	# plt.title("1 A Time-Varying 1 Layer BPA Mu = "+str(mu))
	# plt.show()
