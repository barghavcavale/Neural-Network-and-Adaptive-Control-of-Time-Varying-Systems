...
def Train(mu):
    ...
    for a in range(epochs):
        for i in range(2,end+n):
            ...
            yp[i]=np.sin(mu*(i-1))*f(yp[i-1],yp[i-2])+u
            yp_OSLA[i] = np.sin(mu*(i-1))*f(yp_OSLA[i-1],yp_OSLA[i-2]) + u
    ...
    plt.title("Example 7 Online Time-Varying mu = "+str(mu)+ " Ti = "+str(Ti)+" Tc = "+str(Tc))
    plt.savefig("np90_7a_on_TV_3_2.png")
    plt.show()
    return(W1,W2,W3)
