...
import json
import TrainOSLA_7A
import Train2Layer_Ex7AOff 

mu = 0.1

W1,W2,W3=Train2Layer_Ex7AOff.Train(mu)
W1_OSLA,W2_OSLA=TrainOSLA_7A.Train(mu)
...
# with open("weights_off.json","r") as outfile:
#     W1,W2,W3 = json.load(outfile)

# with open("weights_osla_off.json","r") as outfile:
...
for i in range(2,end+n):
    ...
    ypnc[i]=np.sin(mu*(i-1))*f(ypnc[i-1],ypnc[i-2])+r[i-1]
    ...
    N[i-1] = C
    yp[i] = np.sin(mu*(i-1))*f(yp[i-1],yp[i-2]) - N[i-1] + 0.6*yp[i-1] + 0.2*yp[i-2] + r[i-1]
    ...
    yp3[i] = np.sin(mu*(i-1))*f(yp3[i-1],yp3[i-2]) - fin + 0.6*yp3[i-1] + 0.2*yp3[i-2] + r[i-1]
...
plt.legend(["Plant BPA","Plant OSLA","Model"])
plt.title("Example 7 Offline Time-Varying mu = "+str(mu))
plt.xlabel("Time Steps")
plt.ylabel("Amplitude")
plt.savefig("np90_7a_off_TV_2.png")
plt.show()
