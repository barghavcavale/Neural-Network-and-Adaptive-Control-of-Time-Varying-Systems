...
def Train():
	...
	W2_N1=np.random.normal(0,0.1,(L2,L1+1));
	W2_N2=np.random.normal(0,0.1,(L2,L1+1));


	W3_N1=np.zeros([1,L2+1]);
	W3_N2=np.zeros([1,L2+1]);
	...
	for a in range(epochs):
		for i in range(n,end+n):
			...
			yphat_N1[i]=C_N1+u_N1[i-1];
			e_N1=yphat_N1[i]-yp_N1[i];
			#del2=e*dtan_h(B1);
			del3_N1=e_N1;
			del2_N1=np.matmul((np.transpose(W3_N1[...,1:])),del3_N1)*dtan_h(B1_N1);
			del1_N1=np.matmul((np.transpose(W2_N1[...,1:])),del2_N1)*dtan_h(A1_N1);
			del2_N1=del2_N1.reshape(len(del2_N1),1);
			del1_N1=del1_N1.reshape(len(del1_N1),1);
			#y=y.reshape(1,len(y));
			x_N1=x_N1.reshape(1,len(x_N1));
			y_N1=y_N1.reshape(1,len(y_N1));
			W3_N1=W3_N1-(eta*del3_N1*np.transpose(z_N1));		
			W2_N1=W2_N1-(eta*np.matmul(del2_N1,y_N1));
			...
			yphat_N2[i]=C_N2+u_N2[i-1];
			e_N2=yphat_N2[i]-yp_N2[i];
			#del2=e*dtan_h(B1);
			del3_N2=e_N2;
			del2_N2=np.matmul((np.transpose(W3_N2[...,1:])),del3_N2)*dtan_h(B1_N2);
			del1_N2=np.matmul((np.transpose(W2_N2[...,1:])),del2_N2)*dtan_h(A1_N2);
			del2_N2=del2_N2.reshape(len(del2_N2),1);
			del1_N2=del1_N2.reshape(len(del1_N2),1);
			#y=y.reshape(1,len(y));
			x_N2=x_N2.reshape(1,len(x_N2));
			y_N2=y_N2.reshape(1,len(y_N2));
			W3_N2=W3_N2-(eta*del3_N2*np.transpose(z_N2));      
			W2_N2=W2_N2-(eta*np.matmul(del2_N2,y_N2));
	...
	return(W1_N1,W2_N1,W3_N1,W1_N2,W2_N2,W3_N2);
