...
from Train2Layer_6A import Train 
...
def Run():
	...
	for a in range(epochs):
		for i in range(n,end+n):
			...
			#C=tan_h(C1);
			C=C1;
			yphat[i]=0.8*yphat[i-1]+C;
			e=(yp[i]-yphat[i]);	
			J=J+e*e;
	J=J/(end);

	
	pickle_out=open("BPA2Layer_6A_yphat.pickle","wb");
	pickle.dump(yphat,pickle_out);
	pickle_out.close();
	pickle_out=open("BPA2Layer_6A_J.pickle","wb");
	...
	pickle_out=open("BPA2Layer_6A_J_U.pickle","wb");
	pickle.dump(J_U,pickle_out);
	pickle_out.close();
	pickle_out=open("BPA2Layer_6A_NT.pickle","wb");
