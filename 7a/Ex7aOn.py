...
def Train():
    ...
    for i in range(0,end+n):
        r[i] = np.sin(2*np.pi*(i)/25)

    yphat=np.zeros((end+n,1),dtype='float')
    ...
    W1_OSLA=np.random.normal(0,0.1,(L1,2+1))
    #W2=np.random.normal(0,0.1,(L2,L1+1))
    ...
    for a in range(epochs):
        for i in range(2,end+n):
            ...
            if((i-2)%Tc==0):
                u=-C + 0.6*yp[i-1]+0.2*yp[i-2]+r[i-1]
                u_OSLA=-fin + 0.6*yp_OSLA[i-1]+0.2*yp_OSLA[i-2]+r[i-1]

            yp[i]=f(yp[i-1],yp[i-2])+u
            yp_OSLA[i] = f(yp_OSLA[i-1],yp_OSLA[i-2]) + u
            ec=ym[i]-yp[i]
            #del3=e*dtan_h(C1)
    ...
    print('VAF BPA',vaf(ym[start:end],yp[start:end]))
    print('VAF OSLA',vaf(ym[start:end],yp_OSLA[start:end]))
    plt.legend(["BPA","OSLA","Model"])
    plt.xlabel("Time Steps")
    plt.ylabel("Amplitude")
    #plt.savefig("np90_7b_online_ti10tc10.png")
    plt.savefig("final_7b_online.png")
    #plt.show()
    return(W1,W2,W3)
