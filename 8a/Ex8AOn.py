...
def Train():
    ...
    for a in range(epochs):
        for i in range(n,end+n):
            ...
            yp_OSLA[i]=f(yp_OSLA[i-1],yp_OSLA[i-2],yp_OSLA[i-3])+uc_OSLA[i]+0.8*uc_OSLA[i-1]

            J_OSLA += (yp_OSLA[i]-ym[i])**2
    ...
    Jc=Jc/(end)
    J_OSLA=J_OSLA/end
    print("Cost for BPA 2 Layer Identification = ",Ji)
    print("Cost for BPA 2 Layer Control = ",Jc)
    print("Cost for OSLA = ",J_OSLA)
    print("VAF BPA", vaf(yp,ym))
    print("VAF OSLA", vaf(yp_OSLA,ym))
    ...
    plt.savefig("final_8b_on_ini.png")
    ...
    plt.xlabel("Time Steps")
    plt.ylabel("Amplitude")
    plt.savefig("final_8b_on_fin.png")
    plt.show()
    return(W1,W2,W3)
