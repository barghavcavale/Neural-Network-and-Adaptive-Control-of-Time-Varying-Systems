...
def Train():
    ...
    yp=np.zeros((end+n,2),dtype='float')
    r=np.zeros((end+n,2),dtype='float')
    
	
    for i in range(0,end+n):
        r[i,..] = [np.sin(2*np.pi*(i)/25) np.cos(2*np.pi*(i)/25)] #9a
        # r[i] = np.sin(2*np.pi*(i)/25)+np.sin(2*np.pi*(i)/10) #9b
        

    yphat=np.zeros((end+n,2),dtype='float')
    ym=np.zeros((end+n,2),dtype='float')
    uc=np.zeros((end+n,2),dtype='float')
    ...
    for a in range(epochs):
        for i in range(n,end+n):
            ...
            ym[i,..]=np.matmul(VAL,ym[i-1,..])+r[i-1]
